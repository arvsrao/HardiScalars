/*=========================================================================
  ...
  Module:    $RCSfile: itkScalarGradientNDAnisotropicVectorDiffusionFunctionAverage..txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef __itkScalarGradientNDAnisotropicVectorDiffusionFunctionAverage_txx
#define __itkScalarGradientNDAnisotropicVectorDiffusionFunctionAverage_txx

namespace itk {

template<class TImage>
double ScalarGradientNDAnisotropicVectorDiffusionFunctionAverage<TImage>
::m_MIN_NORM = 1.0e-10;
  
template<class TImage>
ScalarGradientNDAnisotropicVectorDiffusionFunctionAverage<TImage>
...
template<class TImage>
typename ScalarGradientNDAnisotropicVectorDiffusionFunctionAverage<TImage>::PixelType
ScalarGradientNDAnisotropicVectorDiffusionFunctionAverage<TImage>
                ...
                const FloatOffsetType&)
{
  ...
  for (i = 0; i < ImageDimension; i++)
    ...
    accum_d = 0.0;
      ...
      {
      accum+=vnl_math_sqr( dx_forward.GetElement(k) );
      accum_d += vnl_math_sqr( dx_backward.GetElement(k) );
    for (j = 0; j < ImageDimension; j++)
      {
      if (j != i)
        {
        ...
        accum += 0.25f * vnl_math_sqr( dx[j].GetElement(k) + dx_aug.GetElement(k) );
        accum_d += 0.25f * vnl_math_sqr( dx[j].GetElement(k) + dx_dim.GetElement(k) );
	}
        }
      }
    ...
    if (m_K == 0.0)
      ...
      {
       Cx = vcl_exp(accum  / m_K );
      Cxd= vcl_exp( accum_d/ m_K );

  ...
  for (i = 0; i < ImageDimension; i++)
    ...
    for (j = 0; j < ImageDimension; j++)
      ...
      if (j != i)
        ...
        accum += 0.25f * vnl_math_sqr( dx[j].GetElement(this->GetDrivingComponent()) + dx_aug );
        accum_d += 0.25f * vnl_math_sqr( dx[j].GetElement(this->GetDrivingComponent()) + dx_dim );
        }
      }
    ...
    if (m_K == 0.0)
      ...
      {
      Cx = vcl_exp(( vnl_math_sqr( dx_forward.GetElement(this->GetDrivingComponent()) ) + accum)  / m_K );
      Cxd= vcl_exp(( vnl_math_sqr( dx_backward.GetElement(this->GetDrivingComponent())) + accum_d)/ m_K );
      }

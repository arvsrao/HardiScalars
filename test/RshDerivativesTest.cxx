  ...
  for ( int l = 0; l <= int(Order); l+=2)
   {
     ...
     { 
       ...
       if (  temp > epsilon )
       {
           std::cerr<< "Error!! Difference for dP^l_m at " << "l=" << l << " and " << "m=" << m << " is: " <<  temp << ", X value is " << xValues[k] << "\n";
           failure = true;
       }   
       temp = fabs( del_x2_PlmComputed[k][ getIndex(l,m) ] -  del_x2_PlmTest[k][ getIndex(l,m)] );
       if (  temp > epsilon )
       {
           std::cerr<< "Error!! Difference for ddP^l_m at " << "l=" << l << " and " << "m=" << m << " is: " <<  temp << ", X value is " << xValues[k] << "\n";
           failure = true;
       }
     }
   }
